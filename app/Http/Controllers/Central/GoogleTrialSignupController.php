...
class GoogleTrialSignupController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            ...
            'tenant_slug' => ['required', 'string', 'max:60', 'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/', Rule::unique('tenants', 'slug')],
        ...
        if (in_array($data['tenant_slug'], $reserved, true)) {
            throw ValidationException::withMessages(['tenant_slug' => 'Slug tersebut tidak tersedia. Silakan pilih yang lain.']);
        }

        if ($data['plan'] === 'enterprise') {
            return back()->withErrors(['plan' => 'Paket Enterprise perlu proses demo & kontrak. Silakan hubungi kami.'])->withInput();
        }
        ...
        [$tenant, $owner] = DB::transaction(function () use ($data, $google, $plan, $trialEnds) {
            ...
            \App\Models\Outlet::create(['tenant_id' => $tenant->id, 'name' => 'Outlet Utama', 'address' => null, 'is_default' => 1]);
            ...
            foreach (['owner', 'manager', 'cashier', 'kitchen', 'waiter'] as $role) {
                Role::firstOrCreate(['name' => $role, 'guard_name' => 'web', 'tenant_id' => $tenant->id]);
            }
